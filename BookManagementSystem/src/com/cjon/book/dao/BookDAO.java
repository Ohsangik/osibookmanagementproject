...
public class BookDAO {
	...
	public String selectForPage(int page){
		...
		try {
			...
			System.out.println(count+"count 올림 값 출력");
				...
				sql = "select bisbn, bimgbase64, btitle, bauthor, bprice from book limit ?, 12 ";
	...
	// 검색했을때 리스트 띄우는 함수!!
	public String selectAll(String keyword,int page){
		...
		try {
			...
			System.out.println("페이지 번호입니닷 :   "+page);
			//전체 데이터가 얼마나 되지는 count// 확인하고!
			sql = "select count(*) from book where btitle like ?";
			pstmtcount = con.prepareStatement(sql);
			pstmtcount.setString(1, "%"+keyword+"%");
			rscount = pstmtcount.executeQuery();
			while(rscount.next()){
				count = rscount.getInt(1);
				System.out.println(count+"검색한 책의 개수");
			...
			sql = "select bisbn, btitle, bdate, bpage, bprice,bauthor,btranslator,bsupplement,bpublisher,bimgbase64"
					+ " from book where btitle like ? limit ?, 12";
			
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%"+keyword+"%");
			pstmt.setInt(2, pageCal);
			rs = pstmt.executeQuery();
			JSONArray arr = new JSONArray();
	...
	public boolean insert(String imgfile, String isbn, String bookprice, String booktitle,String bookauthor,String bookpub,String booktran){
		...
		try {
			...
			String sql1 = "insert into rentreturn (bisbn, rimg,rtitle,rauthor,rstatus, remail) values ( ?,?,?,?,?,?)"; 
			pstmt1 = con.prepareStatement(sql1);
			pstmt1.setString(1, isbn);
			pstmt1.setString(2, imgfile);
			pstmt1.setString(3, booktitle);
			pstmt1.setString(4, bookauthor);
			pstmt1.setInt(5, 0);
			...
			if(xx==1){
				result = true;
				DBTemplate.commit(con);
			System.out.println("대여관리 테이블에도 인서트가 되었습니다.!!");	
			}else{
				DBTemplate.rollback(con);
			}
	...
	//키워드 검색하면 ~~~~!~!~!~!~!~!~!~!~!~!~!~!~!~ 도서명으로 확인
		public String commentKeywordList(String search, String filter){
		...
		try {
			...
			System.out.println(filter+"리뷰리스트 보기=--------------------------------------------");
			if(filter.equals("ctitle")){
				sql = "select * from books_comment where ctitle like ? order by cid desc";

			}else{
				sql = "select * from books_comment where ctext like ? order by cid desc";
			}
				pstmt = con.prepareStatement(sql);
			...
			System.out.println("00000000000000000000000000000000000000000000===00000000000000000000");
